...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function applyToProduct($productId, $objectToStore) {
        ...
        $productData = $con->fetchRow("select * from {$res->getTableName('catalog_product_entity')} where `entity_id`={$productId}"); //print_r($productData); exit;
        //check for M2 EE constraint
        if (isset($productData['row_id'])) {          
            //$_dummyId = (int)$con->fetchOne("select `row_id` from {$res->getTableName('catalog_product_entity')} where `entity_id`={$productId} and `updated_in` >= unix_timestamp(now()) order by `row_id` desc");
            //$dummyId = $_dummyId ? $_dummyId : $productData['row_id'];
            $dummyId = $productData['row_id'];
        } else {
            $dummyId = $productId;
        }
        ...
        foreach($objectToStore as $storeId => &$object) {
            ...
            foreach($sections as $key => $section) {
                $sectionOrder++;
                if (empty($section) || !is_array($section['fields'])) continue;
            ...
            if ($storeId > 0) {
                ...
                foreach ($sections as &$section) {
                    if (isset($section['fields'])) {
                        foreach ($section['fields'] as $field) if (isset($field['internal_id'])) unset($_defaultStoreFields[(int) $field['internal_id']]);
                    }
                }

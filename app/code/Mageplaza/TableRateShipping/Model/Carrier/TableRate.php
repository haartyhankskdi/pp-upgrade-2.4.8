...
class TableRate extends AbstractCarrier implements CarrierInterface
{
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        foreach ($this->collectionFactory->create()->getItems() as $item) {
            ...
            // Check if the shipping method is excluded for the destination country
            $destinationCountry = $request->getDestCountryId();

            //Fetch the not_applied_countries value from the rate collection
            $notAppliedCountries = [];
            foreach ($rates as $rate) {
                $notAppliedCountries[] = $rate->getData('not_applied_countries');
            }
            $notAppliedCountries = implode(',', array_filter($notAppliedCountries));

            if ($notAppliedCountries && in_array($destinationCountry, explode(',', $notAppliedCountries))) {
                // Skip the rate calculation for excluded countries
                continue;
            }

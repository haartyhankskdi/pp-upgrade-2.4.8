...
class StringUtil extends \Magento\Framework\DataObject
{
     ...
     * @return string
     */
    function getEbayCustomerMessageText($text)
    {
        $matches = [];
        $regex = '#\<div id="UserInputtedText"\>(.+?)\<\/div\>#s';
        ...
        $textReplaced = $this->removeInvisibleContent($match);
     ...
     * @return string
     */
    function getEbaySupportMessageText($text)
    {
        return $this->removeInvisibleContent($text);
    }
     ...
     * @return string
     */
    function removeInvisibleContent ($text)
    {
        $text = preg_replace(
            array(

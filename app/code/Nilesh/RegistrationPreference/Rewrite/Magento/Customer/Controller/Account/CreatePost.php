...
namespace Nilesh\RegistrationPreference\Rewrite\Magento\Customer\Controller\Account;
...
use Sachin\Customer\Helper\Ageverify;
use Sachin\Customer\Helper\Registerverify;
...
class CreatePost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            // echo "<pre>"; var_dump($addresses); exit();
            $customer->setAddresses($addresses);
            $password = $this->getRequest()->getParam('password');
            $confirmation = $this->getRequest()->getParam('password_confirmation');
            $redirectUrl = $this->session->getBeforeAuthUrl();

            //  !This commnet as there is no nned for customer registration
           ...
           // $this->verifyHelper->VerifyAge($_POST['firstname'],$_POST['lastname'],$_POST['gender'],$_POST['street'],$_POST['city'],$_POST['country_id'],$_POST['postcode']);
           //comment below by sachin on 13th May2021

           $this->verifyHelper->VerifyAge($customer->getId(),$_POST['firstname'],$_POST['lastname'],$new_date,$gender_tracemart,$_POST['street'][0],$_POST['street'][1],$_POST['city'],$_POST['country_id'],$_POST['postcode']);
            if ($confirmationStatus === AccountManagementInterface::ACCOUNT_CONFIRMATION_REQUIRED) {
                ...
                if (!$this->scopeConfig->getValue('customer/startup/redirect_dashboard') && $requestedRedirect) {
                    $resultRedirect->setUrl('/register-success');
            ...
            if($this->getRequest()->getParam('refback') == "yes"){
                $this->session->setRefback('yes');
            }
            $product_url = $this->dataHelper->getCatalogSession()->getMyName();
            if(!empty($product_url)){
                    //$prod_url = parse_url($product_url);
                  return $resultRedirect->setUrl('/customer/ageverification');
                    }else{
                  return $resultRedirect->setUrl('/register-success');
                    }
            //return $resultRedirect->setUrl('/register-success');
            return $resultRedirect;
        } catch (StateException $e) {
            /*
                !Nilesh --- Added for purpose
             */
            // $this->messageManager->addExceptionMessage($e, __('Nilesh ').$e->getMessage());
            ...
            $this->messageManager->addExceptionMessage($e, __('We can\'t save the customer.').$e->getMessage());
        }

...
namespace Nilesh\Theme\Rewrite\Magento\Sales\Model\Order\Pdf\Items\Invoice;
...
class DefaultInvoice extends \Magento\Sales\Model\Order\Pdf\Items\Invoice\DefaultInvoice
{
    ...
    public function draw()
    {
        ...
        if ($item->getOrderItem()->getProductOptionByCode('simple_name')) {
            $name = $item->getOrderItem()->getProductOptionByCode('simple_name');
        } else {
            $name = $item->getName();
        }
        ...
        $lines[0] = [
            [
                ...
                'text' => $this->string->split(html_entity_decode($name), 35, true, true),
                'feed' => 35
            ]
        ];
